    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-heading leading-tight">
            <div class="flex space-x-8">
                ...
                <span>{{ __('Buat penjualan baru') }}</span>
            </div>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="mx-auto w-full sm:max-w-md px-6 py-4 overflow-hidden sm:rounded-lg"
                    x-data="{ netto: null, price: null, amount: null }"
                    x-init="fetch('{{ route('getSellPrice') }}').then(response => response.json()).then(data => price = data.sell)">
